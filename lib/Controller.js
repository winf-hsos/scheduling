...
class Controller {
      ...
      fs.readFile('setup/setup.json', 'utf8', function(err, data) {
        ...
        setupInfo.products.forEach(function(product) {
          ...
          product.workplan.forEach(function(step) {
            workplan.addStep(new Workstep(stepNo++, _this.getWorkstationById(step.workstation), step.duration));
          });
  ...
  checkRelease() {
    ...
    // If there are items to releas, relase each
  ...
  /* Releases an order item */
  releaseItem(item) {
    if (item.released === false) {

      // Check if workstation has capacity
      if (item.getFirstWorkstation().canAcceptItem()) {
        item.moveToNextWorksation();
        item.released = true;
  ...
  getWorkstationById(id) {
    var result = this.workstations.filter(function(workstation) {
